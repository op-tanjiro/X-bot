...
const gifted_api = `https://api.giftedtech.my.id/api/`;
let {
  fromBuffer
} = require("file-type");
const {
...
async function instagram(query) {
  const trimmedQuery = query.trim();
  const url = `https://api.giftedtech.my.id/api/download/instadl?apikey=gifted&type=video&url=${encodeURIComponent(trimmedQuery)}`;
