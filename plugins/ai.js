...
smd(
  ...
  async (m) => {
    try {
      ...
      if (!query) {
        return await m.send("Please provide a query, e.g., `.gpt what's good?`.");
      }
...
smd(
  ...
  async (m) => {
    try {
      ...
      // Define the new API URL
      const apiUrl = `https://api.giftedtech.my.id/api/ai/davinci?apikey=gifted&q=${encodeURIComponent(query)}`;
      const response = await fetch(apiUrl);
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("X-bot• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
