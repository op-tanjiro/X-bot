...
smd({
  ...
  try {
    ...
    if (data.status === "true" && data.result.sd) {
      const videoDownloadUrl = data.result.sd;  // Extract the low-quality video URL
...
smd(
    ...
    try {
      ...
      if (!repoUrl) {
        return await _0x1ae8f8.reply(
          "*Provide Repo URL, e.g., .gitclone https://github.com/Mek-d1/X-bot_*"
        );
      }
 ...
 smd({
   pattern: "sound",
    alias: ["X-boti", "aine","mentalism","alive","waso"],
 ...
 smd({
   ...
   try {
     ...
     try {
       ...
       let _0x4b47c3 = "./temp/X-bot-Md " + _0x1d542b[1] + ".mp3";
